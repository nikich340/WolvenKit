    ...
    public class TextureManager : IWitcherArchive
    {
        ...
        public void LoadAll(string exedir)
        {
            ...
            if (Directory.Exists(dlc))
            {
                ...
                foreach (var file in dlcdirs.Where(dir => new Regex("(dlc..)|(dlc.)|(DLC..)|(DLC.)|(BOB)|(ep.)|(bob)|(EP.)").IsMatch(Path.GetFileName(dir ?? ""))).SelectMany(dir => Directory.GetFiles(dir ?? "", "*.cache", SearchOption.AllDirectories).OrderBy(k => k).Where(x => Cache.GetCacheTypeOfFile(x) == Cache.Cachetype.Texture)))
                {
                    LoadBundle(file);
                }
            }
        ...
        public void LoadModsBundles(string exedir)
        {
            ...
            if (Directory.Exists(dlc))
            {
                var dlcdirs = new List<string>(Directory.GetDirectories(dlc));
                dlcdirs.Sort(new AlphanumComparator<string>());
                foreach (var file in dlcdirs.Where(dir => !new Regex("(dlc..)|(dlc.)|(DLC..)|(DLC.)|(BOB)|(bob)|(EP.)|(ep.)").IsMatch(Path.GetFileName(dir ?? ""))).SelectMany(dir => Directory.GetFiles(dir ?? "", "*.cache", SearchOption.AllDirectories).OrderBy(k => k).Where(x => Cache.GetCacheTypeOfFile(x) == Cache.Cachetype.Texture)))
                {
                    LoadModBundle(file);
                }
            }
